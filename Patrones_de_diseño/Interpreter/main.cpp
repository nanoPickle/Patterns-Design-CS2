...
public:
    void interpret(char* input, int& total) override {
        int index = 0;        if (!strncmp(input, nine(), 2)) {
            total += 9 * multiplier();
            index += 2;
...
int main() {
    std::unique_ptr<RNInterpreter> thousands = RNInterpreterFactory::createThousand();
    std::unique_ptr<RNInterpreter> hundreds = RNInterpreterFactory::createHundred();
